...
- `APPOINTMENT_DOCTOR_ASSIGNED` — to the patient when a doctor/slot is assigned. `recipientRole: "PATIENT"`, `data`: `{ appointmentId, doctorId, doctorName, timeSlotId, appointmentDate, scheduledAt, startTime, endTime, hospitalName, serviceType, specialty, patientEmail }`, keys: `notification.patient.doctorAssigned.*`. Idempotency key `APPOINTMENT_DOCTOR_ASSIGNED:<appointmentId>:<recipientEmail>`. The fields `doctorName`, `hospitalName`, `startTime` (epoch ms), `endTime` (epoch ms), `serviceType`, and `specialty` are **new, optional, backward-compatible** (any may be `null`); times remain epoch ms. The fallback `message` is now a complete human-readable sentence (doctor + formatted time + location, no raw epoch) — prefer `messageKey`/`titleKey` + `data` for rendering.
